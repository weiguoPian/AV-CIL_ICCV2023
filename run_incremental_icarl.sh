...
if [ $classifier != NME ] && [ $classifier != FC ];then
    echo "classifier must be \"NME\" or \"FC\".";
    exit;
fi
...
CUDA_VISIBLE_DEVICES=3 nohup python -u train_incremental_icarl.py \
                                --dataset $dataset \
                                ...
                                --gen_exem_batch_size $gen_exem_batch_size > nohup_icarl.log 2>&1 &
