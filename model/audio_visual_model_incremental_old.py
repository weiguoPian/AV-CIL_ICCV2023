...
class IncreAudioVisualNet(nn.Module):
    ...
    def forward(self, visual=None, audio=None, out_features=False, out_both=False, out_feature_before_fusion=False, out_attn_score=False, AFC_train_out=False):
        if self.modality == 'visual':
            if visual is None:
            ...
            if out_features:
                return F.normalize(visual_feature)
            elif out_both:
            ...
            if out_features:
                return F.normalize(audio_feature)
            elif out_both:
            ...
            if out_features:
                return F.normalize(audio_visual_features)
            elif out_both:
                if out_feature_before_fusion:
                    if out_attn_score:
                        return logits, audio_visual_features, F.normalize(audio_feature), F.normalize(visual_feature), spatial_attn_score, temporal_attn_score
                    else:
                        return logits, audio_visual_features, F.normalize(audio_feature), F.normalize(visual_feature)
                else:
                    if out_attn_score:
                        return logits, audio_visual_features, spatial_attn_score, temporal_attn_score
                    else:
                        return logits, audio_visual_features
            elif out_feature_before_fusion:
                if out_attn_score:
                    return logits, F.normalize(audio_feature), F.normalize(visual_feature), spatial_attn_score, temporal_attn_score
                else:
                    return logits, F.normalize(audio_feature), F.normalize(visual_feature)
            elif AFC_train_out:
                audio_feature.retain_grad()
                visual_feature.retain_grad()
        ...
        if out_attn_score:
            return logits, spatial_attn_score, temporal_attn_score
        else:
            return logits
